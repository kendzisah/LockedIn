...
export const defaultSession: SessionState = {
  timerText: '3:42',
  phaseText: 'Settle in. Control your breathing.',
...
export default function SessionTemplate({ state }: Props) {
  return (
...
export function SessionFields({
  ...
  const presets = {
    lock_in: [
      'Settle in. Control your breathing.',
      'Lock in. Silence the noise.',
      'Commit. This is your time.',
      'Stay present. Stay locked.',
      'Discipline is identity.',
      'Finishing strong.',
    ],
    unlock: [
      'Let the noise settle.',
      'Process what you built today.',
      'Observe without judgment.',
      'Release the tension. Keep the lesson.',
      'Acknowledge the work. Let go of the rest.',
      'Reflection complete.',
    ],
  };
  ...
  return (
    <>
      <div className="field-group">
        <div className="field-group-title">Session Mode</div>
        <div className="field">
          <label>Phase</label>
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              style={{
                ...
                padding: '8px 12px',
                borderRadius: 6,
                border: `1px solid ${state.mode === 'lock_in' ? '#00C2FF' : '#1e2330'}`,
                background: state.mode === 'lock_in' ? 'rgba(0,194,255,0.1)' : 'rgba(255,255,255,0.04)',
                color: state.mode === 'lock_in' ? '#00C2FF' : '#6B7280',
            ...
            <button
              style={{
                ...
                padding: '8px 12px',
                borderRadius: 6,
                border: `1px solid ${state.mode === 'unlock' ? '#B0A0FF' : '#1e2330'}`,
                background: state.mode === 'unlock' ? 'rgba(176,160,255,0.1)' : 'rgba(255,255,255,0.04)',
                color: state.mode === 'unlock' ? '#B0A0FF' : '#6B7280',
                fontFamily: "'Inter', sans-serif",
                fontSize: 13,
                cursor: 'pointer',
              }}
      ...
      <div className="field-group">
        ...
        <div className="field">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {presets[state.mode].map((text) => (
              <button
                key={text}
                onClick={() => onChange({ ...state, phaseText: text })}
                style={{
                  padding: '6px 10px',
                  textAlign: 'left',
                  background:
                  ...
                  borderRadius: 6,
                  color: state.phaseText === text ? '#00C2FF' : '#9CA3AF',
                  fontSize: 12,
                  cursor: 'pointer',
                  fontFamily: "'Inter', sans-serif",
                }}
      ...
      <div className="field-group">
        ...
        <div className="field">
          <label>Hold Hint</label>
          <input
            type="text"
            value={state.holdHint}
